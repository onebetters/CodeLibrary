...
public class RedisLock {
    ...
    public boolean tryLock() {
        return Optional.ofNullable(RedisUtils.execute(new DefaultRedisScript<>(SCRIPT_LOCK, Boolean.class),
                                                      Collections.singletonList(key), true, timeout))
                       .orElse(false);
    }
    ...
    public <E extends Throwable> void run(final Runnable action, final Supplier<? extends E> failedExceptionSupplier) throws E {
        if (this.tryLock()) {
            try {
                action.run();
            } finally {
